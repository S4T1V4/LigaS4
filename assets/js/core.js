...
/**
 * Sincroniza alturas de filas entre la parte fija (Pos/Equipo)
 * y la parte scrolleable (PJ→PTS) para el layout "split" en móvil.
 */
function syncTablaSplitHeights() {
  const split = document.getElementById('tablaPosicionesSplit');
  if (!split) return;
  ...
  const leftRows = [
    ...left.querySelectorAll('thead tr'),
    ...left.querySelectorAll('tbody tr'),
  ];
  const rightRows = [
    ...right.querySelectorAll('thead tr'),
    ...right.querySelectorAll('tbody tr'),
  ];
  ...
  // force a reflow so measurements are correct
...
// Decide si usar layout split (tablet/web) cuando hay scroll horizontal.
function toggleTablaSplitLayout() {
    const wrap = document.querySelector('.table-pos-wrap');
    if (!wrap) return;

    // Preferimos split en pantallas “medias” o cuando la tabla se desborda
    const desktopWrap = wrap.querySelector('.tabla-desktop-wrap .table-wrap') || wrap.querySelector('.table-wrap');
    const hasOverflow = !!(desktopWrap && desktopWrap.scrollWidth > desktopWrap.clientWidth + 2);
    const shouldUseSplit = window.innerWidth <= 1100 || hasOverflow;

    wrap.classList.toggle('use-split', shouldUseSplit);
    return shouldUseSplit;
}
...
// ==========================================
// SNAPSHOT (1 request) + cache TTL (modo red lenta)
// Requiere RPC en Supabase: fn_torneo_snapshot(p_torneo_id uuid) -> jsonb
// ==========================================
const __LF_SNAP_TTL_MS = 60 * 1000; // 60s (ajusta a gusto)
const __LF_SNAP_PREFIX = 'lf:snapshot:v2:';
let __lfLastSnapshot = null;
let __lfLastTorneoMeta = null;
let __lfSnapRefreshTimer = null;

// Liguilla tab state (solo lectura): ocultar en público si no existe y promover a primer tab cuando aparezca
let __lfHasLiguilla = null; // null=desconocido, false=no existe, true=existe

function __getTabBtn(tabName) {
  try { return document.querySelector(`.tabs .tab[data-action="tab"][data-tab="${tabName}"]`); } catch { return null; }
}

function __placeTabAfter(tabBtn, afterBtn) {
  if (!tabBtn || !afterBtn) return;
  const parent = afterBtn.parentElement;
  if (!parent) return;
  if (afterBtn.nextSibling) parent.insertBefore(tabBtn, afterBtn.nextSibling);
  else parent.appendChild(tabBtn);
}

function __placeTabFirst(tabBtn) {
  if (!tabBtn) return;
  const parent = tabBtn.parentElement;
  if (!parent) return;
  if (parent.firstElementChild !== tabBtn) parent.insertBefore(tabBtn, parent.firstElementChild);
}

function __setTabVisible(tabBtn, visible) {
  if (!tabBtn) return;
  tabBtn.style.display = visible ? '' : 'none';
  tabBtn.setAttribute('aria-hidden', visible ? 'false' : 'true');
}

function __currentActiveTabName() {
...
/**
 * Reglas (modo público):
 * - Si no hay liguilla (array vacío): ocultar tab.
 * - Si aparece (pasa de vacío -> con datos): mostrar, mover al inicio y abrirla.
 * - Si desaparece (raro): ocultar y volver a Tabla si estaba activa.
 *
 * En admin: el tab siempre visible (porque ahí se genera/programa).
 */
function __updateLiguillaTabStateFromSnapshot(ligArr, { autoFocusOnAppear = true } = {}) {
  const btnLig = __getTabBtn('liguilla');
  if (!btnLig) return;

  // Admin: siempre visible (no escondemos la herramienta de generar/programar)
  if (UI_IS_ADMIN) {
    __setTabVisible(btnLig, true);
    return;
  }

  const has = Array.isArray(ligArr) && ligArr.length > 0;
  const prev = (__lfHasLiguilla === true);
  __lfHasLiguilla = has;

  const btnCal = __getTabBtn('enfrentamientos');

  if (!has) {
    // Ocultar y regresar a la posición "normal" para que setTorneo no marque un tab oculto como primero.
    __setTabVisible(btnLig, false);
    if (btnCal) __placeTabAfter(btnLig, btnCal);

    // Si alguien estaba parado en Liguilla, lo regresamos a Tabla
    const active = __currentActiveTabName();
    if (active === 'liguilla') {
      try { showTab('tabla'); } catch {}
    }
    return;
  }

  // Existe liguilla: mostrar y promover
  __setTabVisible(btnLig, true);
  __placeTabFirst(btnLig);

  // Si apareció por primera vez (o venimos de torneo nuevo), abrirla automáticamente
  if (autoFocusOnAppear && !prev) {
    try { showTab('liguilla'); } catch {}
  }
}


function __snapKey(torneoId) {
    return `${__LF_SNAP_PREFIX}${String(torneoId || '')}`;
}

function __snapRead(torneoId) {
    try {
        const raw = localStorage.getItem(__snapKey(torneoId));
        if (!raw) return null;
...
function __snapWrite(torneoId, data) {
    try {
        const entry = { ts: Date.now(), data };
        localStorage.setItem(__snapKey(torneoId), JSON.stringify(entry));
        return entry;
    } catch {
        return null;
    }
...
function __snapIsFresh(entry, ttlMs = __LF_SNAP_TTL_MS) {
    const ts = Number(entry?.ts || 0);
    if (!ts) return false;
    return (Date.now() - ts) <= Number(ttlMs || 0);
}

function __snapInvalidate(torneoId) {
    try { localStorage.removeItem(__snapKey(torneoId)); } catch {}
}

function __snapPatch(torneoId, patchFn) {
    try {
        const cur = __snapRead(torneoId);
        if (!cur?.data) return;
        const next = patchFn?.(cur.data);
        if (next) __snapWrite(torneoId, next);
    } catch {}
}

function __isMissingRpc(err, fnName) {
    const msg = String(err?.message || err?.details || err?.hint || '').toLowerCase();
    const fn = String(fnName || '').toLowerCase();
    return !!fn && msg.includes(fn) && (msg.includes('function') || msg.includes('rpc'));
}

async function __fetchSnapshot(torneoId) {
    if (!supabaseConfigured || !supabaseClient || !torneoId) return null;

    const { data, error } = await supabaseClient.rpc('fn_torneo_snapshot', {
        p_torneo_id: String(torneoId)
    });

    if (error) throw error;

    // La función debe devolver un JSON (objeto). Si regresa texto, intentamos parsear.
    if (!data) return null;
...
function __normalizePartido(p) {
    if (!p) return null;

    const localId = (p.localId ?? p.local_id ?? p.localid) ? String(p.localId ?? p.local_id ?? p.localid) : null;
    const visitanteId = (p.visitanteId ?? p.visitante_id ?? p.visitanteid) ? String(p.visitanteId ?? p.visitante_id ?? p.visitanteid) : null;

    const localNombre = String(p.local ?? p.local_nombre ?? p.localName ?? '').trim() || (localId ? (equiposById.get(localId) || '—') : '—');
    const visitanteNombre = String(p.visitante ?? p.visitante_nombre ?? p.visitanteName ?? '').trim() || (visitanteId ? (equiposById.get(visitanteId) || '—') : '—');

    const fecha = p.fecha ?? p.fecha_match ?? null;
    const hora = p.hora ?? p.hora_match ?? null;

    const fhRaw = p.fechaHora ?? p.fecha_hora ?? p.fechahora ?? null;
    const fechaHora = fhRaw ? new Date(fhRaw) : (fecha && hora ? new Date(`${fecha}T${hora}`) : null);

    const lat = Number.parseFloat(p.ubicacion_lat ?? p.lat ?? p.ubicacion?.lat);
    const lng = Number.parseFloat(p.ubicacion_lng ?? p.lng ?? p.ubicacion?.lng);
    const ubicacion = (Number.isFinite(lat) && Number.isFinite(lng)) ? { lat, lng } : (p.ubicacion || UBICACION_DEFAULT);

    const finalizado = !!(p.finalizado ?? p.jugado);

    const r = p.resultado || null;
    const golesLocal = Number(r?.golesLocal ?? p.golesLocal ?? p.goles_local ?? 0);
    const golesVisitante = Number(r?.golesVisitante ?? p.golesVisitante ?? p.goles_visitante ?? 0);
    const penalesLocal = Number(r?.penalesLocal ?? p.penalesLocal ?? p.penales_local ?? 0);
    const penalesVisitante = Number(r?.penalesVisitante ?? p.penalesVisitante ?? p.penales_visitante ?? 0);
    ...
    return {
        ...
        fecha,
        hora,
        fechaHora: (fechaHora && !Number.isNaN(fechaHora.getTime())) ? fechaHora : (fecha && hora ? new Date(`${fecha}T${hora}`) : new Date()),
        notas: p.notas ?? '',
        ubicacion,
        finalizado,
        fase: p.fase ?? 'LIGA',
        tipoPartido: p.tipoPartido ?? p.tipo_partido ?? 'OFICIAL',
        resultado: finalizado ? { golesLocal, golesVisitante, penalesLocal, penalesVisitante } : null
    };
}

function applySnapshot(snap) {
    if (!snap || !torneoActualId) return;

    __lfLastSnapshot = snap;
    __lfLastTorneoMeta = snap?.torneo || null;

    // Equipos
    equipos = Array.isArray(snap?.equipos) ? snap.equipos : (Array.isArray(snap?.data?.equipos) ? snap.data.equipos : []);
    equiposById = new Map((equipos || []).map(e => [String(e.id), String(e.nombre || '')]));

    // Partidos
    const rawPartidos = Array.isArray(snap?.partidos) ? snap.partidos : (Array.isArray(snap?.data?.partidos) ? snap.data.partidos : []);
    enfrentamientosProgramados = (rawPartidos || []).map(__normalizePartido).filter(Boolean);

    // Posiciones
    posiciones = Array.isArray(snap?.posiciones) ? snap.posiciones : (Array.isArray(snap?.data?.posiciones) ? snap.data.posiciones : []);
    posicionesByEquipoId = new Map((posiciones || []).map(p => [String(p.equipo_id), p]));

    // Aviso
    const avisoTxt = (snap?.aviso?.texto ?? snap?.aviso_texto ?? snap?.aviso ?? '') ?? '';
    avisoActual = String(avisoTxt || '');

    // TOPs (primea cache para que Resumen/Disciplina no hagan consultas extra)
    const top_goleadores = Array.isArray(snap?.top_goleadores) ? snap.top_goleadores : (Array.isArray(snap?.data?.top_goleadores) ? snap.data.top_goleadores : null);
    const top_amarillas  = Array.isArray(snap?.top_amarillas) ? snap.top_amarillas : (Array.isArray(snap?.data?.top_amarillas) ? snap.data.top_amarillas : null);
    const top_rojas      = Array.isArray(snap?.top_rojas) ? snap.top_rojas : (Array.isArray(snap?.data?.top_rojas) ? snap.data.top_rojas : null);

    const tops = {
        goleadores: Array.isArray(top_goleadores) ? top_goleadores : [],
        amarillas: Array.isArray(top_amarillas) ? top_amarillas : [],
        rojas: Array.isArray(top_rojas) ? top_rojas : [],
    };

    // Solo guardamos si trae algo (evita cachear vacío por error de snapshot)
    if (tops.goleadores.length || tops.amarillas.length || tops.rojas.length) {
        cacheSet(__LF_CACHE.topsByTorneo, String(torneoActualId), tops);
    }


    // Liguilla (primea cache para que pestaña no haga consultas extra)
    const lig = Array.isArray(snap?.liguilla) ? snap.liguilla : (Array.isArray(snap?.data?.liguilla) ? snap.data.liguilla : null);
    if (Array.isArray(lig)) {
        cacheSet(__LF_CACHE.liguillaByTorneo, String(torneoActualId), lig);
    }

    // UX: si no hay liguilla, ocultamos el tab (modo público). Si aparece, se vuelve el primer tab y se abre.
    __updateLiguillaTabStateFromSnapshot(Array.isArray(lig) ? lig : [], { autoFocusOnAppear: true });
}

function scheduleSnapshotRefresh(ms = 1200, reason = 'debounce') {
    try { clearTimeout(__lfSnapRefreshTimer); } catch {}
    __lfSnapRefreshTimer = setTimeout(async () => {
        try {
            await refreshSnapshotAfterMutation({ force: true, silent: true, reason });
        } catch {}
    }, Math.max(200, Number(ms || 0)));
}

async function refreshSnapshotAfterMutation({ force = true, silent = true, debounce = false, ms = 1200, reason = 'mutation' } = {}) {
    if (!torneoActualId) return;

    // Invalidamos snapshot persistido y caches derivadas
    __snapInvalidate(torneoActualId);
    try { __LF_CACHE.topsByTorneo?.delete?.(String(torneoActualId)); } catch {}
    try { __LF_CACHE.statsByTorneo?.delete?.(String(torneoActualId)); } catch {}
    try { __LF_CACHE.liguillaByTorneo?.delete?.(String(torneoActualId)); } catch {}

    if (debounce) {
        scheduleSnapshotRefresh(ms, reason);
        return;
    }

    await cargarDatos({ force: !!force, silent: !!silent, reason });
}
...
function initPromoDelDia() {
  ...
  spot.addEventListener('mouseleave', promoDayStart);
        ...
        async function setTorneo(id, opts = {}) {
            ...
            // En modo público, ocultamos Liguilla hasta que el snapshot confirme que existe.
            // (Evita que quede como "primer tab" activo estando oculto).
            if (!UI_IS_ADMIN) {
                try {
                    __lfHasLiguilla = null;
                    const btnLig = __getTabBtn('liguilla');
                    const btnCal = __getTabBtn('enfrentamientos');
                    if (btnLig) {
                        __setTabVisible(btnLig, false);
                ...
                try {
                    const btnLig = __getTabBtn('liguilla');
                    if (btnLig) __setTabVisible(btnLig, true);
                } catch {}
            }

            document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
            const tabsAll = Array.from(document.querySelectorAll('.tab'));
            const primeraTab = tabsAll.find(t => t.offsetParent !== null) || tabsAll[0] || null;
            ...
            __lfLastSnapshot = null;
            __lfLastTorneoMeta = null;
...
async function crearTorneo(e) {
    ...
    .single();
        ...
        async function cargarDatos(opts = {}) {
            const { force = false, silent = false, reason = '' } = (opts || {});
            if (!supabaseConfigured) {
            ...
            const cached = __snapRead(torneoActualId);

            const paint = async (snap, meta = {}) => {
                applySnapshot(snap);
                ...
                requestAnimationFrame(() => {
                    toggleTablaSplitLayout();
                    syncTablaSplitHeights();
                });

                // Panels (no deberían hacer requests si el snapshot trae stats/posiciones)
                await cargarAdminResumenTorneo();
await cargarPublicSidePanels();

                // Hint si venimos de cache viejo
                if (meta?.fromCache && meta?.stale) {
                    try { console.warn('Snapshot stale:', meta); } catch {}
                }
            };

            // 1) Si hay cache y no es force: pinta rápido y (si está viejo) refresca en background.
            if (!force && cached?.data) {
                try {
                    await paint(cached.data, { fromCache: true, stale: !__snapIsFresh(cached), reason });
                } catch (e) {
                    console.warn('No pude pintar snapshot desde cache:', e);
                }

                if (__snapIsFresh(cached)) return;

                // Background refresh (sin spinner)
                __fetchSnapshot(torneoActualId)
                    .then(snap => {
                        if (!snap) return;
                        __snapWrite(torneoActualId, snap);
                        __lfLastSnapshot = snap;
                        __lfLastTorneoMeta = snap?.torneo || null;
                        return paint(snap, { fromCache: false, stale: false, reason: 'bg-refresh' });
                    })
                    .catch(err => {
                        // Red inestable: nos quedamos con lo que ya teníamos.
                        console.warn('Background snapshot refresh falló:', err);
                    });
            ...
            try {
                const snap = await __fetchSnapshot(torneoActualId);
                if (!snap) throw new Error('Snapshot vacío');

                __snapWrite(torneoActualId, snap);
                __lfLastSnapshot = snap;
                __lfLastTorneoMeta = snap?.torneo || null;

                await paint(snap, { fromCache: false, stale: false, reason });
            } catch (error) {
                // 2) Si falla el fetch, intenta mostrar cache viejo si existe.
                if (cached?.data) {
                    await paint(cached.data, { fromCache: true, stale: true, reason: 'fallback-cache' });
                    showError('Mostrando datos guardados (red inestable).');
                    return;
                }

                // 3) Si no existe la RPC, caemos a modo legacy (varios requests)
                if (__isMissingRpc(error, 'fn_torneo_snapshot')) {
                    console.warn('RPC fn_torneo_snapshot no encontrada; usando modo legacy.');
                    await cargarDatosLegacy();
                    return;
                }
        ...
        async function cargarDatosLegacy() {
            if (!supabaseConfigured) {
                hideLoading();
                return;
            }
...
await cargarPublicSidePanels();
       ...
       async function cargarPartidos() {
        ...
        function showTab(tabName, e) {
            ...
            const targetId = String(tabName || '').trim();
            if (!targetId) return;

            // Ocultar/mostrar secciones (y mantener accesibilidad)
            const sections = [...document.querySelectorAll('.section')];
            sections.forEach(s => {
                const isActive = (s.id === targetId);
                s.classList.toggle('active', isActive);
                // hidden ayuda a SR + evita tabbing a contenido oculto
                s.hidden = !isActive;
                s.setAttribute('aria-hidden', isActive ? 'false' : 'true');
            });

            // Marcar tab activo
            const tabs = [...document.querySelectorAll('.tabs .tab[data-action="tab"]')];
            ...
            const btn = btnFromEvent || document.querySelector(`.tab[data-action="tab"][data-tab="${targetId}"]`);

            tabs.forEach(t => {
                const isActive = (String(t.dataset.tab || '') === targetId);
                t.classList.toggle('active', isActive);
                t.setAttribute('aria-selected', isActive ? 'true' : 'false');
                t.setAttribute('tabindex', isActive ? '0' : '-1');
            });

            // Si el panel existe, actualizamos aria-labelledby/controls por si el HTML no los trae
            try {
                if (btn && targetId) {
                    const panel = document.getElementById(targetId);
                    if (panel) {
                        // Normaliza ids esperados
...
// Liguilla (solo lectura)
            if (targetId === 'liguilla') {
                cargarLiguilla();
            }

            // Calendario: respetar filtro actual
            if (targetId === 'enfrentamientos') {
                filtrarPartidos(filtroActual);
            }
        }
        ...
        async function agregarEquipo(e) {
            ...
            try {
                ...
                // Snapshot: invalida y refresca en debounce para evitar múltiples requests
                await refreshSnapshotAfterMutation({ debounce: true, ms: 1000, reason: 'agregarEquipo' });
                actualizarVistas();
            } catch (error) {
        ...
        async function guardarCambiosEquipoAdmin() {
            ...
            if (equipoModalModo === 'create' || !equipoModalId) {
                ...
                try {
                    ...
                    await onEquipoJugadoresChange(String(nuevo.id));
...
// ==========================================
        ...
        async function configurarEnfrentamiento(e) {
           ...
           if (localId === visitanteId) {
            ...
            try {
                ...
                await refreshSnapshotAfterMutation({ debounce: true, ms: 800, reason: 'programarPartido' });
                actualizarProximoPartido();
                actualizarListaEnfrentamientos();
                actualizarTimelineMini();
        ...
        async function eliminarEnfrentamiento(index) {
            ...
            try {
                ...
                await refreshSnapshotAfterMutation({ debounce: true, ms: 800, reason: 'eliminarPartido' });
                actualizarProximoPartido();
                actualizarListaEnfrentamientos();
                actualizarTimelineMini();
            } catch (error) {
        ...
        async function abrirModalEditarPorPartidoId(partidoId) {
            ...
            if (idx < 0) {
                ...
                await cargarDatos({ force: true, silent: true, reason: 'openById' });
        ...
        async function guardarEdicion(e) {
			...
			if (finalizadoNuevo) {
				...
				if (esLiguilla && empate) {
					...
					marcadorFinal = { golesLocal: gl, golesVisitante: gv, penalesLocal: pl, penalesVisitante: pv };
...
await upsertMarcadorPartidoEnBD(partido.id, {
			...
			// En BD nueva se usa "finalizado". Si aún no existe (legacy), caemos a "jugado".
				...
				// Marcador en memoria (ya lo calculamos/guardamos arriba)
				partido.resultado = finalizadoNuevo ? (marcadorFinal || partido.resultado || null) : null;

				// Refrescar snapshot (1 request) para recalcular tabla/estadísticas/paneles
				await refreshSnapshotAfterMutation({ force: true, silent: true, reason: 'guardarEdicion' });
        ...
        // - Izquierda: top 3 goleadores + mejor ofensiva
        ...
        function hasPublicPanels() {
            ...
            set('sideBestOffense', '<div class="empty-state">—</div>');
            set('sideBestDefense', '<div class="empty-state">—</div>');
            set('mobileTopScorers', '<div class="empty-state">—</div>');
            set('mobileBestOffense', '<div class="empty-state">—</div>');
            set('mobileBestDefense', '<div class="empty-state">—</div>');
// Panel derecho: patrocinadores se pueden renderizar aunque no haya torneo
        ...
        function renderTop3To(containerId, rows, kind) {
            ...
            el.innerHTML = top.map((r, i) => {
                ...
                function resolveBestOffensiveTeam() {
            // 1) Preferimos las posiciones oficiales (si existen)
            const pos = Array.isArray(posiciones) ? posiciones : [];
            if (pos.length) {
                const best = pos
                    .map(p => {
                        const id = String(p.equipo_id ?? '');
                        const nombre = String((p.equipo_nombre ?? '').trim() || equiposById.get(id) || '—');
                        const gf = Number(p.gf ?? 0);
                        const pts = Number(p.pts ?? 0);
                        const dif = Number(p.dif ?? (Number(p.gf ?? 0) - Number(p.gc ?? 0)));
                        const pj = Number(p.pj ?? 0);
                        return { id, nombre, gf, pts, dif, pj };
                    })
                    .filter(x => x.nombre && x.nombre !== '—')
                    .sort((a, b) => (b.gf - a.gf) || (b.pts - a.pts) || (b.dif - a.dif) || a.nombre.localeCompare(b.nombre))[0];

                if (best) return best;
            }

            // 2) Fallback: calcular desde partidos jugados (OFICIAL) si no hay posiciones
            const partidos = Array.isArray(enfrentamientosProgramados) ? enfrentamientosProgramados : [];
            const gfByTeam = new Map();

            partidos
                .filter(p => p && (p.finalizado || p.resultado))
                ...
                .forEach(p => {
                    ...
                    if (local) gfByTeam.set(local, (gfByTeam.get(local) || 0) + gl);
                    if (vis) gfByTeam.set(vis, (gfByTeam.get(vis) || 0) + gv);
                });

            if (gfByTeam.size) {
                const best = [...gfByTeam.entries()]
                    .map(([nombre, gf]) => ({ id: '', nombre, gf, pts: null, dif: null, pj: null }))
                    .sort((a, b) => (b.gf - a.gf) || a.nombre.localeCompare(b.nombre))[0];
        ...
        function renderBestOffenseTo(containerId) {
            const el = document.getElementById(containerId);
            if (!el) return;

            const best = resolveBestOffensiveTeam();
            if (!best) {
                el.innerHTML = '<div class="empty-state">Sin datos.</div>';
                return;
            }

            const title = escapeHtml(best.nombre || '—');
            const gf = Number(best.gf ?? 0);
            const value = `GF ${gf}`;

            const metaParts = [];
            if (best.pj != null && !Number.isNaN(Number(best.pj)) && Number(best.pj) > 0) metaParts.push(`PJ ${Number(best.pj)}`);
            if (best.pts != null && !Number.isNaN(Number(best.pts))) metaParts.push(`PTS ${Number(best.pts)}`);

            const meta = metaParts.length ? metaParts.join(' • ') : 'Más goles a favor';
            ...
            el.innerHTML = `
                <div class="side-item">
                    ...
                    <div class="row2">${escapeHtml(meta)}</div>
                </div>
            `;
        }


        function resolveBestDefensiveTeam() {
            // 1) Preferimos las posiciones oficiales (si existen)
            const pos = Array.isArray(posiciones) ? posiciones : [];
            if (pos.length) {
                const best = pos
                    .map(p => {
                        const id = String(p.equipo_id ?? '');
                        const nombre = String((p.equipo_nombre ?? '').trim() || equiposById.get(id) || '—');
                        const gc = Number(p.gc ?? 0); // goles en contra
                        const pts = Number(p.pts ?? 0);
                        const dif = Number(p.dif ?? (Number(p.gf ?? 0) - Number(p.gc ?? 0)));
                        const pj = Number(p.pj ?? 0);
                        return { id, nombre, gc, pts, dif, pj };
                    })
                    .filter(x => x.nombre && x.nombre !== '—' && x.pj > 0)
                    // Menos GC, luego más PTS, luego mejor DIF
                    .sort((a, b) => (a.gc - b.gc) || (b.pts - a.pts) || (b.dif - a.dif) || a.nombre.localeCompare(b.nombre))[0];
            ...
            // 2) Fallback: calcular desde partidos jugados (OFICIAL) si no hay posiciones
            const partidos = Array.isArray(enfrentamientosProgramados) ? enfrentamientosProgramados : [];
            const gcByTeam = new Map();
            const pjByTeam = new Map();
            ...
            partidos
                ...
                .forEach(p => {
                    ...
                    if (local) {
                        gcByTeam.set(local, (gcByTeam.get(local) || 0) + gv); // local recibió gv
                        pjByTeam.set(local, (pjByTeam.get(local) || 0) + 1);
                    }
                    if (vis) {
                        gcByTeam.set(vis, (gcByTeam.get(vis) || 0) + gl);   // visitante recibió gl
                        pjByTeam.set(vis, (pjByTeam.get(vis) || 0) + 1);
                    }
                });

            if (gcByTeam.size) {
                const best = [...gcByTeam.entries()]
                    .map(([nombre, gc]) => ({ id: '', nombre, gc, pj: pjByTeam.get(nombre) || 0 }))
                    .filter(x => x.pj > 0)
                    .sort((a, b) => (a.gc - b.gc) || (b.pj - a.pj) || a.nombre.localeCompare(b.nombre))[0];
        ...
        function renderBestDefenseTo(containerId) {
            const el = document.getElementById(containerId);
            if (!el) return;

            const best = resolveBestDefensiveTeam();
            if (!best) {
                el.innerHTML = '<div class="empty-state">Sin datos.</div>';
                return;
            }
            ...
            const meta = metaParts.length ? metaParts.join(' • ') : 'Menos goles recibidos';
            ...
            try {
                ...
                const torneoKey = String(torneoActualId || '');

                // TOPs vienen del snapshot/cache (no stats completos)
                const cachedTops = cacheGet(__LF_CACHE.topsByTorneo, torneoKey, 10 * 60 * 1000);

                const snapMatch = (String(__lfLastSnapshot?.torneo?.id || '') === torneoKey) ? __lfLastSnapshot : null;
                const snapTops = snapMatch ? {
                    goleadores: Array.isArray(snapMatch?.top_goleadores) ? snapMatch.top_goleadores : (Array.isArray(snapMatch?.data?.top_goleadores) ? snapMatch.data.top_goleadores : []),
                    amarillas: Array.isArray(snapMatch?.top_amarillas) ? snapMatch.top_amarillas : (Array.isArray(snapMatch?.data?.top_amarillas) ? snapMatch.data.top_amarillas : []),
                    rojas: Array.isArray(snapMatch?.top_rojas) ? snapMatch.top_rojas : (Array.isArray(snapMatch?.data?.top_rojas) ? snapMatch.data.top_rojas : []),
                } : { goleadores: [], amarillas: [], rojas: [] };

                const tops = cachedTops || snapTops || { goleadores: [], amarillas: [], rojas: [] };

                // TOP goleadores (Resumen)
                renderTop3To('sideTopScorers', tops.goleadores, 'goles');
                renderTop3To('mobileTopScorers', tops.goleadores, 'goles');

                renderBestOffenseTo('sideBestOffense');
                renderBestOffenseTo('mobileBestOffense');

                renderBestDefenseTo('sideBestDefense');
                renderBestDefenseTo('mobileBestDefense');

                // “Última actualización” (del snapshot)
                const genAt = snapMatch ? (snapMatch?.generated_at || snapMatch?.generatedAt || null) : null;
                ...
                try {
                    if (genAt) {
                        ...
                        if (!Number.isNaN(d.getTime())) {
                            updated = `Actualizado: ${d.toLocaleString('es-MX', { dateStyle: 'medium', timeStyle: 'short' })}`;
                        }
                    }
                } catch {}

                const u1 = document.getElementById('sideUpdatedAt');
                if (u1) u1.textContent = updated;

                const u2 = document.getElementById('mobileUpdatedAt');
                if (u2) u2.textContent = updated;
        ...
        function __getTopsForTorneo(torneoId) {
            const torneoKey = String(torneoId || '');
            const cached = cacheGet(__LF_CACHE.topsByTorneo, torneoKey, 10 * 60 * 1000);
            if (cached) return cached;

            const snapMatch = (String(__lfLastSnapshot?.torneo?.id || '') === torneoKey) ? __lfLastSnapshot : null;
            if (!snapMatch) return { goleadores: [], amarillas: [], rojas: [] };
            ...
            // cachear si trae algo
            if (tops.goleadores.length || tops.amarillas.length || tops.rojas.length) {
                cacheSet(__LF_CACHE.topsByTorneo, torneoKey, tops);
            }
        ...
        function openDisciplinaModal() {
            ...
            // Render inmediato
            try { renderDisciplinaModal(); } catch {}
        }
        ...
        function renderDisciplinaModal() {
            const tops = __getTopsForTorneo(torneoActualId);

            // Amarillas / Rojas (Top 3)
            renderTopLista('disciplinaAmarillas', Array.isArray(tops?.amarillas) ? tops.amarillas : [], {
                valueKey: 'amarillas',
                labelPrefix: '🟨 ',
                emptyText: 'Sin datos.',
            });

            renderTopLista('disciplinaRojas', Array.isArray(tops?.rojas) ? tops.rojas : [], {
                valueKey: 'rojas',
                labelPrefix: '🟥 ',
                emptyText: 'Sin datos.',
            });
    ...
    const torneo = (__lfLastTorneoMeta && String(__lfLastTorneoMeta.id || '') === String(torneoActualId))
        ? __lfLastTorneoMeta
        : null;

    const fase = formatFaseLabel(torneo?.fase_actual || torneo?.fase || torneo?.estado || '');

    const pendientesArr = (enfrentamientosProgramados || [])
        .filter(p => !p.finalizado && String(p.tipoPartido || 'OFICIAL').toUpperCase() === 'OFICIAL');

    const pendientes = pendientesArr.length;

    const next = pendientesArr
        .filter(p => p?.fechaHora)
        .slice()
        .sort((a, b) => new Date(a.fechaHora) - new Date(b.fechaHora))[0] || null;

    const nextTxt = next
        ? `${next.local} vs ${next.visitante} · ${formatFechaHoraBonita(next.fechaHora)}`
        : '—';

    const campeonId = torneo?.campeon_id ?? torneo?.campeonId ?? torneo?.campeon ?? null;
    const subId = torneo?.subcampeon_id ?? torneo?.subcampeonId ?? torneo?.subcampeon ?? null;

    const campeonNombre = campeonId ? (equiposById.get(String(campeonId)) || '—') : 'Pendiente';
    const subNombre = subId ? (equiposById.get(String(subId)) || '—') : '';

    const campeonTxt = campeonId
        ? `${campeonNombre}${subId ? ` / ${subNombre}` : ''}`
        : 'Pendiente';

    wrap.innerHTML = `
        <div class="summary-item"><div class="k">📌 Fase</div><div class="v">${escapeHtml(fase || '—')}</div></div>
        <div class="summary-item"><div class="k">⏳ Pendientes (oficial)</div><div class="v">${pendientes}</div></div>
        <div class="summary-item"><div class="k">🗓️ Próximo</div><div class="v">${escapeHtml(nextTxt)}</div></div>
        <div class="summary-item"><div class="k">🏆 Campeón</div><div class="v">${escapeHtml(campeonTxt)}</div></div>
    `;
}
...
function limpiarEstadisticasUI() {
        ...
        async function renderCampeonEnEstadisticas() {
            ...
            const torneoNombre =
                String(__lfLastTorneoMeta?.nombre || '') ||
                String((torneos.find(t => String(t.id) === String(torneoActualId))?.nombre) || '');

            const campeonId = __lfLastTorneoMeta?.campeon_id ?? __lfLastTorneoMeta?.campeonId ?? __lfLastTorneoMeta?.campeon ?? null;
            const subId = __lfLastTorneoMeta?.subcampeon_id ?? __lfLastTorneoMeta?.subcampeonId ?? __lfLastTorneoMeta?.subcampeon ?? null;
            ...
            const campeon = equiposById.get(String(campeonId)) || 'Campeón';
            const sub = subId ? (equiposById.get(String(subId)) || 'Subcampeón') : '';
...
async function obtenerEstadisticasJugadores() {
    ...
    if (!torneoActualId) return null;

    const rows = Array.isArray(posiciones) ? posiciones : [];
    if (rows.length) {
        const isDef = String(kind || '').toLowerCase().includes('def');
        let best = null;

        for (const r of rows) {
            const gf = Number(r.goles_a_favor ?? r.golesFavor ?? r.goles_favor ?? 0);
            const gc = Number(r.goles_en_contra ?? r.golesContra ?? r.goles_contra ?? 0);

            if (!best) {
                best = { ...r, goles_a_favor: gf, goles_en_contra: gc };
                continue;
            }

            if (isDef) {
                // Mejor defensa: menor GC, desempate mayor GF
                if (gc < best.goles_en_contra || (gc === best.goles_en_contra && gf > best.goles_a_favor)) {
                    best = { ...r, goles_a_favor: gf, goles_en_contra: gc };
                }
            } else {
                // Mejor ofensiva: mayor GF, desempate menor GC
                if (gf > best.goles_a_favor || (gf === best.goles_a_favor && gc < best.goles_en_contra)) {
                    best = { ...r, goles_a_favor: gf, goles_en_contra: gc };
                }
            }
        }
        ...
        // Normaliza nombre si el snapshot no lo trae
        const equipoNombre = best.equipo_nombre || equiposById.get(String(best.equipo_id)) || 'Equipo';

        return {
            ...best,
            equipo_nombre: equipoNombre,
            goles_a_favor: Number(best.goles_a_favor ?? 0),
            goles_en_contra: Number(best.goles_en_contra ?? 0)
        };
    }

    // Fallback (si no hay posiciones en memoria)
    if (!supabaseConfigured) return null;

    const orderColumn = (kind === 'defensa') ? 'goles_en_contra' : 'goles_a_favor';
    const asc = (kind === 'defensa');

    try {
        const { data, error } = await supabaseClient
            .from('v_posiciones')
            .select('equipo_nombre, goles_a_favor, goles_en_contra')
            .eq('torneo_id', torneoActualId)
        ...
        console.warn('No se pudo obtener mejor equipo desde posiciones:', err);
        return null;
    }
}
        ...
        async function cargarLiguilla({ force = false } = {}) {
            ...
            const tId = String(torneoActualId);
            const LIGUILLA_TTL_MS = 10 * 60 * 1000; // 10 min (evita recargas al cambiar de tab)

            // Si el snapshot ya trajo liguilla, aquí solo pintamos del cache y salimos (sin requests)
            const cached = (!force) ? cacheGet(__LF_CACHE.liguillaByTorneo, tId, LIGUILLA_TTL_MS) : null;
            if (Array.isArray(cached)) {
                if (cached.length === 0) {
                    ...
                    setLiguillaGeneradaUI(true, cached);
                }
                return;
            }
            ...
            try {
                ...
                if (!data || data.length === 0) {
                    wrap.innerHTML = '<div class="empty-state">Aún no hay liguilla generada para este torneo.</div>';
                    setLiguillaGeneradaUI(false);
                    cacheSet(__LF_CACHE.liguillaByTorneo, tId, []);
                    return;
                }
                ...
                // Guardar cache
                cacheSet(__LF_CACHE.liguillaByTorneo, tId, data);
...
async function generarLiguillaDesdeUI() {
            ...
            try {
                ...
                await refreshSnapshotAfterMutation({ force: true, silent: true, reason: 'liguillaGenerada' });
                actualizarProximoPartido();
                actualizarListaEnfrentamientos();
                actualizarTimelineMini();
            } catch (err) {
        ...
        async function programarRondaActivaLiguilla() {
            ...
            try {
                ...
                const cached = cacheGet(__LF_CACHE.liguillaByTorneo, String(torneoActualId), 10 * 60 * 1000);
                const data = Array.isArray(cached) ? cached : (await (async () => {
                    const { data, error } = await supabaseClient
                        ...
                        .order('llave', { ascending: true });
                    if (error) throw error;
                    cacheSet(__LF_CACHE.liguillaByTorneo, String(torneoActualId), data || []);
                    return data || [];
                })());
                ...
                await refreshSnapshotAfterMutation({ force: true, silent: true, reason: 'liguillaProgramarRonda' });
                actualizarProximoPartido();
                actualizarListaEnfrentamientos();
                actualizarTimelineMini();
            } catch (e) {
        ...
        async function agregarEventoPartido() {
            ...
            try {
                ...
                await cargarEventosPartido();
                // Stats/tabla/paneles: refresco con debounce para no saturar en red lenta
                await refreshSnapshotAfterMutation({ debounce: true, ms: 1200, reason: 'evento+add' });
            } catch (err) {
        ...
        async function eliminarEventoPartido(eventoId) {
            ...
            try {
                ...
                renderEventosPartido();
                // Stats/tabla/paneles: refresco con debounce para no saturar en red lenta
                await refreshSnapshotAfterMutation({ debounce: true, ms: 1200, reason: 'evento+del' });
            } catch (err) {
        ...
        async function guardarAviso(e) {
            ...
            try {
                if (texto) {
                    ...
                    avisoActual = '';
                    vista.textContent = 'Sin avisos publicados';
                }
        ...
        // ===== Aviso (Modal en modo espectador) =====
function __avisoHash(str) {
    try {
        let h = 0;
        const s = String(str || '');
...
function __getAvisoSeenKey() {
    const tid = torneoActualId ? String(torneoActualId) : 'none';
    const h = __avisoHash(avisoActual || '');
    return `lf_aviso_seen_${tid}_${h}`;
}

function openAvisoModal(force = false) {
    if (UI_IS_ADMIN) return;

    const modal = document.getElementById('modalAvisoLiga');
    const body = document.getElementById('avisoModalBody');
    if (!modal || !body) return;

    if (!avisoActual) return;

    body.textContent = avisoActual;

    modal.classList.add('show');
    modal.setAttribute('aria-hidden', 'false');

    // (Opcional) enfocar botón para UX
    const ok = document.getElementById('avisoModalOk');
    if (ok) {
...
function initAvisoModal() {
    ...
    const btnX = document.getElementById('avisoModalClose');
    const btnOk = document.getElementById('avisoModalOk');

    if (btnX) btnX.addEventListener('click', closeAvisoModal);
    if (btnOk) btnOk.addEventListener('click', closeAvisoModal);

    // Click fuera del contenido cierra
    modal.addEventListener('click', (e) => {
        if (e.target === modal) closeAvisoModal();
    });

    // Escape cierra
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') closeAvisoModal();
    });

    // Botón (icono) como gatillo (modo espectador)
    const btn = document.getElementById('btnMostrarAviso');
    if (btn) {
        btn.addEventListener('click', () => openAvisoModal(true));
    }
}

function mostrarAvisoBanner() {
    // Admin conserva banner, Público usa modal + icono
    const btn = document.getElementById('btnMostrarAviso');
    ...
    // Público
    if (btn) {
        btn.disabled = !avisoActual;
    }

    if (avisoActual) {
        // Mostrar SIEMPRE al cargar/revisitar
        openAvisoModal(true);
    } else {
        closeAvisoModal();
    }
...
// FUNCIONES DE VISUALIZACIÓN
        ...
        function actualizarListaEnfrentamientos() {
            ...
            div.innerHTML = ordenados.map((e) => {
                ...
                const notasHtml = e.notas ? (() => {
                    const safe = escapeHtml(String(e.notas || '')).replace(/\r?\n/g, '<br>');
                    return `
                ...
                if (tieneResultado) {
                    const gl = Number(e?.resultado?.golesLocal ?? 0);
                    const gv = Number(e?.resultado?.golesVisitante ?? 0);

                    let localCls = 'draw';
                    let visCls = 'draw';
                    if (gl > gv) { localCls = 'win'; visCls = 'lose'; }
                    else if (gl < gv) { localCls = 'lose'; visCls = 'win'; }

                    contenidoPrincipal = `
                        <div class="enfrentamiento-equipos">
                            <span class="team ${localCls}">${escapeHtml(e.local)}</span>
                            <span class="vs">VS</span>
                            <span class="team ${visCls}">${escapeHtml(e.visitante)}</span>
                        </div>
                        <div class="resultado-final">
                            ${gl} - ${gv}
                        </div>
                        <div class="enfrentamiento-hora">
                            📅 ${escapeHtml(fechaFormato)} | ⏰ ${escapeHtml(horaFormato)}
                        </div>
                    `;
                } else {
                    contenidoPrincipal = `
                        <div class="enfrentamiento-equipos">
                            <span class="team">${escapeHtml(e.local)}</span>
                            <span class="vs">VS</span>
                            <span class="team">${escapeHtml(e.visitante)}</span>
                        </div>
                        <div class="enfrentamiento-hora">⏰ ${escapeHtml(horaFormato)}</div>
                    `;
                }
        ...
        function actualizarTabla() {
            ...
            if (tbodyFixed || tbodyScroll) {
                ...
                requestAnimationFrame(() => {
                    // Activa split en tablet/web si aplica y sincroniza alturas
                    const usingSplit = toggleTablaSplitLayout();
                    if (usingSplit || window.innerWidth <= 900) syncTablaSplitHeights();
                });
...
export async function initApp() {
    ...
    initAvisoModal();      // Avisos como modal (modo espectador)
