...
export function bindEvents(core) {
  ...
  document.addEventListener('click', (ev) => {
    ...
    switch (action) {
      ...
      case 'equipo-ajuste-open': {
        const id = el.dataset.id || '';
        const nombre = el.dataset.nombre || '';
        // usa core directo para evitar ReferenceError por versiones mezcladas
        if (core?.abrirModalEquipoAjuste) core.abrirModalEquipoAjuste(id, nombre);
        break;
      }
      case 'equipo-ajuste-cerrar':
        // cerrar modal ajuste (usa core directo para evitar ReferenceError por versiones mezcladas)
        if (core?.cerrarModalEquipoAjuste) core.cerrarModalEquipoAjuste();
        break;
      case 'equipo-ajuste-guardar':
        if (core?.guardarAjusteEquipoPagado) core.guardarAjusteEquipoPagado('set');
        break;
      case 'equipo-ajuste-quitar':
        if (core?.guardarAjusteEquipoPagado) core.guardarAjusteEquipoPagado('clear');
        break;
  ...
  closeOnBackdropClick('modalEquipoAjuste', () => {    if (core?.cerrarModalEquipoAjuste) core.cerrarModalEquipoAjuste();  });
